...
// 
// Parser rules begin with a lower case letter, lexer rules begin with an Uppercase letter. To
// create a parser rule, write the name followed by a colon (:) and then a list of alternatives,
// separated by pipe (|) characters. You can use parenthesis for sub-expressions, alternatives
// within those sub-expressions, and kleene * or + on any element in a rule.
// 
// Every production rule corresponds to a class that gets generated in the target language for the
// ANTLR generator. If we use alternative labels, as in `type`, then subclasses of the rule-class
// will be created for each label. If one alternative is labelled, then they all must be. The
// purpose of labels is to call different functions in the generated listeners and visitors for the
// results of these productions.
// 
// Lastly, ANTLR's DSL contains a feature that allows us to name the matched tokens and productions
// in an alternative (name=part) or collect multiple tokens or productions of the same type into a
// list (list+=part). The `type` production below uses this feature, too.
...
type_rule:
	SEQ LBRACK type_rule RBRACK												# SeqType
	| SET LBRACK type_rule RBRACK											# SetType
	| MAP LBRACK keyType = type_rule COMMA valueType = type_rule RBRACK		# MapType
	| LPAREN tupTypes += type_rule (COMMA tupTypes += type_rule)* RPAREN	# TupleType
...
idenType: name = iden COLON type_rule;

funParamList: funParam (COMMA funParam)*;
funParam: name = iden COLON type_rule;
...
typeDefDecl:
	TYPE name = iden SEMI						# ForeignTypeDef
	| TYPE name = iden ASSIGN type_rule SEMI	# PTypeDef;

enumTypeDefDecl:
	ENUM name = iden LBRACE enumElemList RBRACE
...
eventDecl:
	EVENT name = iden cardinality? (COLON type_rule)? SEMI;
cardinality: ASSERT IntLiteral | ASSUME IntLiteral;
...
interfaceDecl:
	INTERFACE name = iden LPAREN type_rule? RPAREN (
		RECEIVES nonDefaultEventList?
	) SEMI;
...
receivesSends:
	RECEIVES eventSetLiteral? SEMI	# MachineReceive
	| SENDS eventSetLiteral? SEMI	# MachineSend;

specMachineDecl:
	SPEC name = iden OBSERVES eventSetLiteral machineBody;

machineBody: LBRACE machineEntry* RBRACE;
machineEntry: varDecl | funDecl | group | stateDecl;

varDecl: VAR idenList COLON type_rule SEMI;

funDecl:
	FUN name = iden LPAREN funParamList? RPAREN (COLON type_rule)? (
		CREATES interfaces += iden
	)? SEMI # ForeignFunDecl
	| FUN name = iden LPAREN funParamList? RPAREN (
		COLON type_rule
	)? functionBody # PFunDecl;

group: GROUP name = iden LBRACE groupItem* RBRACE;
groupItem: stateDecl | group;

stateDecl:
	START? temperature = (HOT | COLD)? STATE name = iden LBRACE stateBodyItem* RBRACE;

stateBodyItem:
	ENTRY anonEventHandler									# StateEntry
...
statement:
	...
	| SEND machine = expr COMMA event = expr (COMMA rvalueList)? SEMI	# SendStmt
...
lvalue:
	...
	| lvalue DOT int_rule		# TupleLvalue
...
expr:
	...
	| expr DOT field = int_rule								# TupleAccessExpr
	...
	| fun = DEFAULT LPAREN type_rule RPAREN					# KeywordExpr
	...
	| expr cast = (AS | TO) type_rule						# CastExpr
...
floatLiteral:
	pre = IntLiteral? DOT post = IntLiteral							# DecimalFloat
...
namedTupleBody:
	names += iden ASSIGN values += rvalue COMMA
	| names += iden ASSIGN values += rvalue (
		COMMA names += iden ASSIGN values += rvalue
	)+;
...
modExpr:
	LPAREN modExpr RPAREN													# ParenModuleExpr
...
testDecl:
	TEST testName = iden (
		LBRACK MAIN ASSIGN mainMachine = iden RBRACK
	) COLON modExpr SEMI # SafetyTestDecl
	| TEST testName = iden (
		LBRACK MAIN ASSIGN mainMachine = iden RBRACK
	) COLON modExpr REFINES modExpr SEMI # RefinementTestDecl;

implementationDecl:
	IMPLEMENTATION implName = iden (
		LBRACK MAIN ASSIGN mainMachine = iden RBRACK
	)? COLON modExpr SEMI;